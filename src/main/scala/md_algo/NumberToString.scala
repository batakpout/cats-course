...
def challengeFunction(i: Long) = {
 val b = 1000000000
 val m = 1000000
 val t = 1000
 val h = 100

 def g(n:Long, b:Long, t: String) = {
  val m=n%b
  val d=n/b
 ...
 def f(n: Long, p: Boolean = true): String = {
  if (n>=b) g(n, b, "billion")
   ...
   case r => s"${f(r).stripSuffix("t")}ty ${f(n % 10,false)}"
  }
  else {
   n match {
 ...
 (s.substring(0, 1).toUpperCase() + s.substring(1))
  .replaceAll("( )+", " ")
		...
		println(challengeFunction(999,999,999,999).replaceAll("( )+", " "))
		println("=========")
